...
youtubePlayerBtn.addEventListener('click', () => {
    if (currentPlayer !== 'youtube') {
        ...
        if (customPlayer && customPlayer.video) {
            const currentTime = customPlayer.video.currentTime || 0;
            customPlayer.video.pause();
            customPlayer.video.removeAttribute('src');
            customPlayer.video.load();
        }
...
function initCustomPlayer(startTime = 0) {
    ...
    if (!customPlayer) {
        customPlayer = new CustomVideoPlayer(container, {
            autoplay: true,
            muted: false
        });
    }
    ...
    if (videoId) {
        customPlayer.loadVideo(`videos/${videoId}.mp4`);
        if (startTime > 0) {
            customPlayer.video.currentTime = startTime;
        }
    }
}
